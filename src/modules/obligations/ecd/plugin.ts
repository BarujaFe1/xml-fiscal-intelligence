...
export const ECD_SOURCE_ID = "official:sped:ecd:pending-registry";

const DISCLAIMER =
  "ECD assistida (esqueleto). Plano de contas e lançamentos NÃO são derivados de XML fiscal. Contas DEMO devem ser substituídas por dados contábeis reais antes de qualquer uso no PVA ECD.";
...
function detect(context: ObligationContext): RequiredDataResult {
  ...
  const items = [
    ...
    },
    {
      ...
      status: hasAccountant ? ("complete" as const) : ("blocking" as const),
    ...
    },
    {
      ...
      message: "XML fiscal não gera lançamentos — esqueleto usa plano DEMO",
    },
    {
      id: "demo_chart",
      label: "Plano de contas DEMO incluso",
      status: "review" as const,
      message: "Substituir antes de uso real",
    },
  ];
...
function build(context: ObligationContext): ObligationBuildResult {
  const warnings = [
    "Plano de contas DEMO — não usar em produção.",
    "Sem lançamentos I200 a partir de NF-e (proposital).",
    "Indicação de livro G — ajustar tipo de ECD no PVA.",
  ];
  ...
  // Zero balances for demo accounts — explicit
  ...
  records.push({ type: "I990", fields: [String(3 + DEMO_CHART.length + 1 + DEMO_CHART.filter((a) => a.code.includes(".")).length + 1)] });
...
function validate(build: ObligationBuildResult): ValidationResult {
  ...
  issues.push({
    ...
    message: "Arquivo contém marcações DEMO — não transmitir",
  });
